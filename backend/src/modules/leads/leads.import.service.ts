...
export async function importLeadsFromFile(req: Request, file: Express.Multer.File): Promise<LeadImportResult> {
  ...
  const idx = {
    ...
    sdr: findColumn(headers, ['Lead Generated By', 'SDR', 'SDR Name']),
    createdBySdr: findColumn(headers, ['Created By']),
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    try {
      ...
      // "Lead Generated By" defaults to the importing/assigned rep when the file has no distinct column.
      const sdrName = cell(row, idx.sdr);
      const sdrId = sdrName ? (userIdByName.get(sdrName.toLowerCase()) ?? repId) : repId;
      // "Created By" has no default — it's an optional manual attribution, left unset when the
      // file doesn't name a matching user (unlike "Lead Generated By", which always falls back to
      // the importing rep).
      const createdByName = cell(row, idx.createdBySdr);
      const createdBySdrId = createdByName ? userIdByName.get(createdByName.toLowerCase()) ?? null : null;
