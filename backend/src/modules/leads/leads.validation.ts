...
export const createLeadSchema = z.object({
  ...
  // supplies this value is populated live from /users/assignable?roles=INSIDE_SALES. Labeled
  // "Lead Generated by" on the form.
  sdrId: z.string().uuid().optional().nullable(),
  // "Created By" dropdown — same live INSIDE_SALES source as sdrId, but a distinct field: it's a
  // manually-selected attribution, not the system-audit createdById (which is always whoever is
  // actually logged in and submitting the form).
  createdBySdrId: z.string().uuid().optional().nullable(),
...
export const listLeadsQuerySchema = paginationSchema.extend({
  ...
  createdBySdrId: z.string().uuid().optional(),
