...
describe('Lead Creation module — new fields end to end', () => {
  ...
  it('creates a lead using every new field at once', async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...
        createdBySdrId: insideSalesUserId,
    ...
    expect(res.body.data.createdBySdr.id).toBe(insideSalesUserId);
  ...
  it('filters the leads list by createdBySdrId', async () => {
    const res = await request(app).get(`/api/leads?createdBySdrId=${insideSalesUserId}`).set('Authorization', `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data.some((l: any) => l.id === leadId)).toBe(true);
  });
  ...
  it('CSV export includes the new columns (Industry, Website, Revenue, Lead Generated By, Created By, Lead Received Date, Email Response)', async () => {
    const res = await request(app).get('/api/reports/leads/export.csv').set('Authorization', `Bearer ${adminToken}`);
    expect(res.status).toBe(200);
    const header = res.text.split('\n')[0];
    for (const col of ['Industry', 'Website', 'Revenue', 'Lead Generated By', 'Created By', 'Lead Received Date', 'Email Response', 'State']) {
...
describe('Lead Creation module — Edit Lead form parity with New Lead form', () => {
  ...
  it('Edit Lead can update sdrId, createdBySdrId, leadReceivedDate, campaign, and Email Response in the same request as company/meeting fields', async () => {
